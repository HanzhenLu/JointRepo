...
def test(benchmark:Benchmarks, names:List[str], generator:Generator, retriever:Retriever, args, step:int) -> dict:
    ...
    for name, features in all_eval_examples.items():
        ...
        if name == "cceval_python" or name == "cceval_python_only_prefix":
            ...
            raise Exception(f"unsupport test set {name}")
        logger.info(f"{name} step {step}: {str(results)}")
        table[name] = results
...
def main():
    ...
    parser.add_argument("--generator_type", type=str, required=True, choices=["llama", "deepseek", "Qwen"])
    ...
    if args.only_prefix:
        test_dataset = {
            "ours_suffix_only_prefix": load_dataset("ours-suffix_only_prefix", args.sampled_code_num*5),
            "ours_only_prefix": load_dataset("ours_only_prefix", args.sampled_code_num*5),
            "cceval_python_only_prefix": load_dataset("cceval_only_prefix", args.sampled_code_num*5),
            "repoeval_line_only_prefix": load_dataset("repoeval_only_prefix", args.sampled_code_num*5)
        }
    else:
        test_dataset = {
        ...
        for epoch in range(1, args.num_train_epochs+1):
            for batch in train_dataloader:
                ...
                if len(losses) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.do_valid and args.gradient_accumulation_steps % 1000 == 0:
                        results_table = test(benchmark, ["validation"] + list(test_dataset.keys()), generator, retriever, args, scheduler._step_count)
    ...
    if args.do_test and not args.do_valid:
        test(benchmark, list(test_dataset.keys()), generator, retriever, args, 0 if not args.do_train else epoch)
